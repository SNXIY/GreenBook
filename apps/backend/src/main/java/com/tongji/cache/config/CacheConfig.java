...
public class CacheConfig {
    ...
    public Cache<String, FeedPageResponse> feedPublicCache(CacheProperties props) {
        return Caffeine.newBuilder()//底层concurrenthashmap 存入键值对
                .maximumSize(props.getL2().getPublicCfg().getMaxSize())//最多缓存 1000 个条目，超过时使用 LRU 淘汰
                .expireAfterWrite(Duration.ofSeconds(props.getL2().getPublicCfg().getTtlSeconds()))//过期时间15s
                .build();
    }
