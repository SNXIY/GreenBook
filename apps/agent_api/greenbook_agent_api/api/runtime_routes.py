...
from greenbook_agent_core.execution.event_stream import subscribe_execution_events
    ...
    return ExecutionListResponse(
        items=[
            ExecutionListItem(
                ...
                control_state=execution.control_state.value,
                control_reason=execution.control_reason,
            )
...
async def pause_execution(
    execution_id: str,
    request: Request,
    control: ExecutionControlRequest | None = None,
) -> ExecutionStatusResponse:
    if getattr(request.app.state, "execution_dispatch_mode", None) == "direct":
        raise HTTPException(
            status_code=409,
            detail="Durable pause/resume requires GREENBOOK_AGENT_EXECUTION_DISPATCH=queue",
        )
    return await _control_execution(
        request,
        execution_id,
        lambda manager, value: manager.pause_execution(
            value,
            reason=(control.reason if control and control.reason else "User requested pause"),
        ),
    )
...
async def resume_execution(execution_id: str, request: Request) -> ExecutionStatusResponse:
    ...
    response = await _control_execution(
        request,
        execution_id,
        lambda manager, value: manager.resume_execution(value),
    )
    queue = getattr(request.app.state, "execution_queue", None)
    if queue is None:
        # Compatibility for embedded/in-memory callers. Production queue mode
        # always supplies the durable queue and confirms resume in Worker.
        manager = _manager(request)
        return _status_response(manager._state.confirm_resume(execution_id))
    message = queue.get_by_execution_id(execution_id)
    if message is None:
        raise HTTPException(
            status_code=409,
            detail="Execution has no durable dispatch message and cannot be resumed",
        )
    ...
    if queued.status != ExecutionQueueStatus.READY:
        raise HTTPException(
            status_code=503,
            detail=f"Execution resume could not enter queue: {queued.status.value}",
        )
    return response
...
async def cancel_execution(
    ...
    return await _control_execution(
        request,
        execution_id,
        lambda manager, value: manager.cancel_execution(
            value,
            reason=(
                control.reason
...
@router.get("/executions/{execution_id}/control", response_model=ExecutionControlResponse)
...
def _status_response(execution) -> ExecutionStatusResponse:
    total = execution.total_step_count
    ...
    return ExecutionStatusResponse(
        ...
        control_state=execution.control_state.value,
        control_reason=execution.control_reason,
        control_requested_at=execution.control_requested_at,
    )
...
async def retry_execution_step(
    ...
    if queue is not None:
        ...
        if message is None:
            raise HTTPException(
                status_code=409,
                detail="Execution has no durable dispatch message and cannot be retried",
            )
        queued = queue.enqueue(
            execution_id,
            trace_id=message.trace_id,
        ...
        if queued.status != ExecutionQueueStatus.READY:
            raise HTTPException(
                status_code=503,
                detail=f"Execution retry could not enter queue: {queued.status.value}",
            )
