...
# MCP tools that duplicate legacy ToolRegistry tools (same business capability).
# The legacy tool is the canonical one; the MCP variant must not be registered
# so the LLM Planner is never faced with two tools for the same operation.
_MCP_DUPLICATE_BLOCKLIST: frozenset[str] = frozenset(
    {
        "content.revise_draft",  # legacy: creator.revise_draft
    }
)
        ...
        for server in self.servers:
            ...
            for tool in result.tools:
                if tool.name not in server.allowed_tools:
                    continue
                if tool.name in _MCP_DUPLICATE_BLOCKLIST:
                    continue
