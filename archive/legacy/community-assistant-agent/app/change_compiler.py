...
class ChangeCompiler:
    ...
    def compile(
        ...
        *,
        ...
        # Content improvement with LLM-detected need_reference — builds a
        # deterministic search → revise → (schedule rebind) DAG instead of
        # delegating to the LLM Planner (which would hit operation-contract
        # rejections for search_posts on content-mutation operations).
        if (
            turn_plan.need_reference
            and content is not None
            and content.op in {"APPEND", "REPLACE", "UPDATE_TITLE"}
        ):
            return self._compile_content_improvement(
                content=content,
                schedule=schedule,
                target_context=target_context,
                intent=intent,
    ...
    def _compile_content_improvement(
        self,
        *,
        content: Any,
        schedule: Any | None,
        ...
        """Build a search → revise → (schedule rebind) DAG for content improvement.

        When the LLM detects need_reference, this compiler inserts a community
        search step that feeds POST_SEARCH_RESULTS into creator.revise_draft via
        the expanded references binding.  The rest of the DAG is identical to
        _compile_content_mutation.
        """
        content_target = target_context.content_target
        if content_target is None or content_target.target_type != "DRAFT":
            return None
        ...
        search_query = message.strip()[:200]
        steps: list[AgentPlanStep] = [
            AgentPlanStep(
                task_id="search-reference-posts",
                agent="SearchAgent",
                primary_capability="search",
                capabilities=["search"],
                tool="community.search_posts",
            ...
            AgentPlanStep(
                ...
                label="核验当前帖子草稿",
                arguments={"draft_id": content_target.target_id},
                depends_on=["search-reference-posts"],
                expected_artifact_type="content_draft",
                max_attempts=2,
            ),
            AgentPlanStep(
                ...
                label="参考社区内容修订帖子",
                arguments={"instruction": instruction},
                depends_on=["search-reference-posts", "read-current-draft"],
                expected_artifact_type="content_draft",
                max_attempts=2,
            ),
        ]
        ...
        required = ["search", "rewrite_content"]
        if schedule_target is not None:
