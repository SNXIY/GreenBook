...
class CommunityIntent(ApiModel):
    ...
    # Semantic content intent — filled by the LLM, not by keyword matching.
    # MODIFY: add/change specific things (e.g. "增加代码示例", "改标题")
    # IMPROVE: enhance quality with or without external references
    # REWRITE: restructure or redo entirely
    # POLISH: minor refinements, wording, formatting
    content_intent: Literal["MODIFY", "IMPROVE", "REWRITE", "POLISH"] | None = None
    # Whether the user expects the system to gather external references before
    # generating the revision (e.g. search community posts, analyse engagement).
...
class AdaptiveRoutingDecision(ApiModel):
    ...
    open_plan: bool | None = None
    follow_up_prompts: list[str] = Field(default_factory=list, max_length=3)
    # When the user asks to preview/review before executing, the model sets
    # this to PREVIEW.  The Worker routes to DIRECT with a plain-text
    # preview; no tools run and no side effects occur.
    interaction_mode: Literal["EXECUTE", "PREVIEW"] = "EXECUTE"
