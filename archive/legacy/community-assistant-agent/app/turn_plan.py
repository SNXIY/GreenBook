...
class TurnPlan(ApiModel):
    ...
    # Carried from LLM semantic output: true when the user wants external
    # references gathered before revision (e.g. search → improve → update).
...
_OPERATION_TO_CHANGES: dict[str, list[tuple[ChangeRole, ChangeOp]]] = {
    ...
    "IMPROVE_CONTENT": [("CONTENT", "APPEND")],
...
def changes_from_operation(
    ...
    *,
    ...
    if (
        operation in {"CREATE_POST", "APPEND_CONTENT", "REPLACE_CONTENT", "UPDATE_TITLE", "IMPROVE_CONTENT"}
...
def intent_delta_from_turn_plan(
    *,
    ...
    if (
        ...
        and operation
        in {"APPEND_CONTENT", "REPLACE_CONTENT", "UPDATE_TITLE", "IMPROVE_CONTENT", "UPDATE_SCHEDULE"}
    ):
        if operation == "IMPROVE_CONTENT":
            pass  # preserve IMPROVE_CONTENT — set earlier by need_reference gate
        elif content_change.op == "APPEND":
    ...
    # LLM-detected content improvement with external references.
    # Override the keyword-parsed operation so the ChangeCompiler can build a
    # deterministic search → revise DAG instead of falling to OPEN_PLAN.
    if (
        content_change is not None
        and content_change.op in {"APPEND", "REPLACE", "UPDATE_TITLE"}
        and turn_plan.need_reference
    ):
    ...
    delta: dict[str, Any] = {
        ...
        "open_plan": turn_plan.open_plan,
        "need_reference": turn_plan.need_reference,
    }
...
class TurnPlanBuilder:
    ...
    def from_turn_intent(
        ...
        *,
        ...
        elif operation in {"APPEND_CONTENT", "REPLACE_CONTENT", "UPDATE_TITLE", "IMPROVE_CONTENT"}:
        ...
        # Content improvement with LLM-detected need_reference does NOT insert an
        # ANALYSIS change (which would force CONTINUE_ANALYSIS → open_plan).
        # Instead the flag is carried on TurnPlan and compiled deterministically
        # by ChangeCompiler into a search → revise → (schedule) DAG.
