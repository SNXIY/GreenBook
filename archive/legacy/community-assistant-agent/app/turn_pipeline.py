...
class TurnPipeline:
    ...
    def interpret(
        ...
        *,
        ...
        turn_plan = self.turn_plan_builder.build(
            ...
            need_reference=decision.intent.need_reference,
        )
        primary_text = (
    ...
    def bind_and_compile(
        ...
        *,
        ...
        open_plan = turn_plan.open_plan or (
            intent_delta.operation in {
                "OPEN_PLAN",
                "REPLY_COMMENT",
                "CONTINUE_ANALYSIS",
            }
            or (
                intent_delta.operation == "CREATE_POST"
                and not turn_plan.changes
            )
        )
        # IMPROVE_CONTENT carries a deterministic search→revise DAG;
        # it must not be forced into open_plan.
        if intent_delta.operation == "IMPROVE_CONTENT":
            open_plan = False
        compiled = None
        if not open_plan:
