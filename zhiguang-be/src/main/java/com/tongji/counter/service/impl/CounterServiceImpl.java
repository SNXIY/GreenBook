...
public class CounterServiceImpl implements CounterService {
    ...
    private boolean toggle(String etype, String eid, long uid, String metric, int idx, boolean add) {
        ...
        if (ok) {
            ...
            //CounterEvent.of(etype, eid, metric, idx, uid, delta)这是CounterEvent对象
    ...
    private static final String TOGGLE_LUA = """
            local bmKey = KEYS[1]  --bm:like...
            local offset = tonumber(ARGV[1]) -- 分片内位偏移
            local op = ARGV[2] -- 'add' or 'remove' -- add or remove
            local prev = redis.call('GETBIT', bmKey, offset)--拿到这一位的数据
            if op == 'add' then
