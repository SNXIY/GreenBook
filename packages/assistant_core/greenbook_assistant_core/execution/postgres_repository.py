...
class PostgresExecutionRepository:
    ...
    def find_by_id(self, execution_id: str) -> PlanExecution | None:
        ...
        return _to_execution(execution_row, step_rows)
...
def _to_execution(row: Any, step_rows: list[Any]) -> PlanExecution:
    steps = []
    ...
    return PlanExecution.model_validate({**dict(row), "status": row["status"], "steps": steps})
