"""Repository layer for Agent persistence — SQLAlchemy Core + asyncpg.
...
_conversations = sa.Table(
    ...
    sa.Column("timezone", sa.String(64), nullable=False, default="Asia/Shanghai"),
...
_messages = sa.Table(
    ...
    sa.Column("parts", JSONB, nullable=False, default=list),
...
class ConversationRepository:
    ...
    async def ensure_tables(self) -> None:
        await _ensure_tables(self._session)
    ...
    async def find_by_conversation(
        ...
        *,
        ...
        if limit is not None:
            stmt = stmt.order_by(_messages.c.created_at.desc()).limit(limit)
        else:
...
class ContextRepository:
    """Persistence boundary for long-lived conversation context.

    Context is stored on the conversation aggregate so active Task/Artifact
    references and the summary update atomically with the conversation version.
    """
    ...
    async def get(self, conversation_id: str) -> dict[str, Any] | None:
        ...
        result = row.first()
        return _row_to_dict(result, time_cols=("created_at", "updated_at")) if result else None

    async def update(self, conversation_id: str, **fields: Any) -> dict[str, Any] | None:
        invalid = sorted(set(fields) - self._FIELDS)
        if invalid:
            raise ValueError(f"CONTEXT_FIELDS_NOT_ALLOWED:{','.join(invalid)}")
