...
class ResultResolver:
    ...
    def resolve(
        ...
        *,
        ...
        resolved.artifacts = _complete_empty_terminal_mutation_status(
            resolved,
            list(resolved.artifacts),
        )
...
def _artifact_payload(artifact: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "draft_id": projection.get("draft_id"),
...
def _safe_artifact_payload(raw: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        "draft_id": _optional_text(raw.get("draft_id") or data.get("draft_id")),
        "resource_type": resource_type,
...
def _complete_empty_terminal_mutation_status(
    result: RuntimeResult,
    artifacts: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Fill the verified postcondition omitted by a successful 204 cancel.

    Java's schedule-cancel endpoint intentionally has no response body.  The
    successful terminal step is nevertheless the verified business fact, so a
    schedule artifact without a status must not be projected as an unknown
    resource.  This is limited to the existing CANCEL_SCHEDULE capability and
    never infers a status for a generic or unsuccessful result.
    """

    status = str(result.status or "").upper()
    if status != "COMPLETED" or not result.success:
        return artifacts
    capabilities = {
        str(step.get("capability") or step.get("semantic_action") or "").upper()
        for step in (result.steps or [])
        if isinstance(step, dict)
    }
    if "CANCEL_SCHEDULE" not in capabilities:
        return artifacts
    return [
        {
            **item,
            "status": "CANCELLED"
            if _kind(item) == "SCHEDULE" and not item.get("status")
...
def _step_payload(step: Any) -> dict[str, Any]:
    ...
    goal_id = str(getattr(step, "goal_id", "") or "") or (
