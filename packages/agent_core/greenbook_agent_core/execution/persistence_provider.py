...
class RuntimePersistenceFactory:
    ...
    def from_env(
        ...
        *,
        ...
        configured_storage = storage or os.getenv("GREENBOOK_AGENT_RUNTIME_STORAGE")
        if configured_storage:
            ...
            # ``GREENBOOK_AGENT_DATABASE_URL``.  Treat a configured database as the
        ...
        if selected != cls.POSTGRES:
            raise ValueError(
                "GREENBOOK_AGENT_RUNTIME_STORAGE must be 'memory' or 'postgres'"
            )

        runtime_url = database_url or cls._database_url()
        if bind is None and not runtime_url:
            raise RuntimeError(
                "GREENBOOK_AGENT_RUNTIME_STORAGE=postgres requires "
                "GREENBOOK_AGENT_RUNTIME_DATABASE_URL or GREENBOOK_AGENT_DB_URL"
            )
    ...
    def _postgres(
        ...
        *,
        ...
        return RuntimePersistence(
            ...
            result_projection_store=PostgresExecutionResultProjectionStore(
                bind,
                create_tables=create_tables,
            ),
            bind=bind,
