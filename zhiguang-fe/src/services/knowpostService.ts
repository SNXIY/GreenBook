...
export const knowpostService = {
  ...
  publish: (id: string) =>
    apiFetch<PublishStatusResponse>(`${KNOWPOST_PREFIX}/${id}/publish`, {
      method: "POST"
    }),

  publishStatus: (id: string) =>
    apiFetch<PublishStatusResponse>(`${KNOWPOST_PREFIX}/${id}/publish-status`),

  taskItems: (accessToken: string) =>
    apiFetch<PostTaskItem[]>(`${KNOWPOST_PREFIX}/task-items?limit=30`, { accessToken }),

  content: (id: string, accessToken: string) =>
    apiFetch<string>(`${KNOWPOST_PREFIX}/${id}/content`, { accessToken }),
  ...
  recommendFeed: (page = 1, size = 20) =>
    apiFetch<FeedResponse>(`${KNOWPOST_PREFIX}/feed/recommend?page=${page}&size=${size}`)
  ,

  followingFeed: (page = 1, size = 20, accessToken: string) =>
    apiFetch<FeedResponse>(`${KNOWPOST_PREFIX}/feed/following?page=${page}&size=${size}`, {
      accessToken
    })
  ,
...
export async function uploadToPresigned(putUrl: string, headers: Record<string, string>, file: File) {
  ...
  try {
    const target = new URL(putUrl, window.location.href);
    if (target.origin === window.location.origin || target.pathname.endsWith("/api/v1/storage/local-upload")) {
    ...
    // Keep the storage-supplied headers for external presigned URLs.
  }
