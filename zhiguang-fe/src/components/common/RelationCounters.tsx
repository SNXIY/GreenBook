    ...
    <>
      {counts ? (
        <div className={styles.wrapper}>
          <button type="button" className={`${styles.item} ${styles.clickable}`} onClick={() => { setMode("following"); setOpen(true); }}>
            <span className={styles.number}>{counts.followings}</span>
            <span className={styles.label}>关注</span>
          </button>
          <button type="button" className={`${styles.item} ${styles.clickable}`} onClick={() => { setMode("followers"); setOpen(true); }}>
