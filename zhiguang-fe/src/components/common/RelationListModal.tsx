...
const RelationListModal = ({ open, onClose, userId, mode }: RelationListModalProps) => {
  ...
  return (
    <div className={styles._overlay_1q1ln_1}>
      <div className={styles._modal_1q1ln_12} role="dialog" aria-modal="true" aria-label={title}>
            ...
            <div className={styles._list_1q1ln_52}>
              {profiles.map((p) => (
                <div key={p.id} className={styles._item_1q1ln_59}>
                  {p.avatar ? (
                    <img className={styles._avatar_1q1ln_69} src={p.avatar} alt={p.nickname} width="44" height="44" loading="lazy" />
                  ) : (
                    <div className={styles._avatar_1q1ln_69}>{initialChar(p.nickname, p.id)}</div>
                  )}
                  <div className={styles._name_1q1ln_80}>{p.nickname || "GREEN-BOOK 用户"}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className={styles._footer_1q1ln_85}>
          <button type="button" className={styles._more_1q1ln_93} onClick={loadMore} disabled={!hasMore || loading}>
