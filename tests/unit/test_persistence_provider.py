...
from greenbook_agent_core.execution.retry_task import RetryTask
...
def test_postgres_profile_requires_explicit_database_configuration(monkeypatch) -> None:
    monkeypatch.delenv("GREENBOOK_AGENT_RUNTIME_DATABASE_URL", raising=False)
    monkeypatch.delenv("GREENBOOK_AGENT_DATABASE_URL", raising=False)
    monkeypatch.delenv("GREENBOOK_AGENT_DB_URL", raising=False)
...
def test_database_url_alias_selects_durable_profile_by_default(monkeypatch) -> None:
    monkeypatch.delenv("GREENBOOK_AGENT_RUNTIME_STORAGE", raising=False)
    monkeypatch.delenv("GREENBOOK_AGENT_RUNTIME_DATABASE_URL", raising=False)
    monkeypatch.setenv("GREENBOOK_AGENT_DATABASE_URL", "sqlite+pysqlite:///:memory:")
...
def test_explicit_memory_profile_wins_over_database_url(monkeypatch) -> None:
    monkeypatch.setenv("GREENBOOK_AGENT_DATABASE_URL", "postgresql://ignored")
