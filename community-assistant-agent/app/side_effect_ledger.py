    ...
    async def finish(
        ...
        *,
        ...
        async with self.database.sessions() as session, session.begin():
            ...
            # Release revise claim only when Creator was never submitted.
            # If remote_operation_id is set (or ledger has task_id), keep the
            # durable claim even on FAILED — a second revise must not bypass.
            if (
                ...
                creator_was_submitted = bool(
                    effect.remote_operation_id
                    or ledger.get("remote_task_id")
                    or ledger.get("task_id")
                )
                if not creator_was_submitted:
